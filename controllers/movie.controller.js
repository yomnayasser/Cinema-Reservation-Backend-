...
class Movie {
  ...
  static editmovie = async (req, res) => {
    ...
    catch(e){  res.status(500).send({ apiStatus: false, error: e, message: e.message });}
  };
  ...
  static addActor=async(req,res)=>{
    try{
      ...
      await movie.save()
      res.status(200).send({data:movie, apiStatus:true, message:"Actor Added"})
    }
    catch(e){  res.status(500).send({ apiStatus: false, error: e, message: e.message });}
  }
  static addDirector=async(req,res)=>{
    try{
      ...
      await movie.save()
      res.status(200).send({data:movie, apiStatus:true, message:"Director Added"})
    }
    catch(e){  res.status(500).send({ apiStatus: false, error: e, message: e.message });}
  }
  static addWriter=async(req,res)=>{
    try{
      ...
      await movie.save()
      res.status(200).send({data:movie, apiStatus:true, message:"Writer Added"})
    }
    catch(e){  res.status(500).send({ apiStatus: false, error: e, message: e.message });}
  }
